...
func main() {
	...
	case "4b":
		...
		_ = s[0]
	case "5":
		// Big map, interface in the value
		...
		_ = m[0]
	case "5a":
		// Big map, interface in the value,map fully pre-allocated
		m := make(map[interface{}]interface{}, N) //5a,6a,7a 说明interface{}会别扫描
		for i := 0; i < N; i++ {
			n := int32(i)
			m[n] = n
		}
	...
	case "8a":
		...
		m := make(map[t]int32, N) //这个跟map[int32]int32一样，不会扫描
		for i := 0; i < N; i++ {
			n := int32(i)
			m[t{n, n}] = n
		}
